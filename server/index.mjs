...
export function createShareServer({
  directory = process.env.SHARE_DIRECTORY ?? './data/shares',
  limit = 10000,
} = {}) {
  const locks = new Map();
  ...
  const body = async (req) => {
    ...
    for await (const chunk of req) {
      size += chunk.length;
      if (size > MAX_BYTES)
        throw Object.assign(new Error('分享文档不能超过 8 MB'), { status: 413 });
      chunks.push(chunk);
    }
    let value;
    try {
      ...
      throw Object.assign(new Error('请求不是合法 JSON'), { status: 400 });
    }
    ...
    if (
      ...
      throw Object.assign(new Error('文档结构无效'), { status: 400 });
    return doc;
  };
  ...
  return createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET') {
        ...
        if (!Number.isSafeInteger(version) || version < 1 || version > meta.version) {
          json(res, 404, { error: '分享版本不存在' });
          return;
        }
        const data = JSON.parse(await readFile(join(path, `${version}.json`), 'utf8'));
        json(res, 200, { ...data, latestVersion: meta.version });
        return;
      }
      if (req.method !== 'PUT') {
      ...
      try {
        ...
        if (!timingSafeEqual(Buffer.from(digest(token)), Buffer.from(meta.tokenHash))) {
          json(res, 403, { error: '需要有效的编辑链接才能更新分享' });
          return;
        }
        if (Number(req.headers['if-match']) !== meta.version) {
          json(res, 409, { error: '分享已有新版本，请先加载最新版本', version: meta.version });
          return;
        }
