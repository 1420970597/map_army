...
function SingleFeaturePanel({
  ...
  return (
    <div className="panel-body">
      <div className="symbol-editor-tabs" role="tablist" aria-label="符号编辑器视图">
        {(
          [
            ['edit', '编辑'],
            ['preview', '预览'],
      ...
      {editorTab === 'edit' ? (
                ...
                <input
                  ...
                  onBlur={(e) => {
                    ...
                    if (values.every((n) => Number.isFinite(n) && n > 0))
                      onUpdate(feature.id, {
                        rangeRings: [...new Set(values)].sort((a, b) => a - b),
                      });
                  }}
                />
              </label>
            ...
            <fieldset disabled={nativeExternal} className="native-sidc-fields">
              ...
              <label className="field">
                ...
                <select
                  ...
                  value={sidc.context}
                  onChange={(event) =>
                    patchSidc({ context: Number(event.target.value) as Context })
                  }
                >
          ...
          <div className="panel-section">
            ...
            <label className="field">
              ...
              <input
                ...
                onChange={(event) =>
                  onUpdate(feature.id, { direction: Number(event.target.value) })
                }
              />
            </label>
            <label className="field">
          ...
          {canResetBearing && (
            <div className="panel-section vertex-bearing-actions">
              <button
                ...
                onClick={() =>
                  activeVertex !== null && onResetVertexBearing(feature.id, activeVertex)
                }
              >
                重置当前顶点方向
              </button>
        ...
        <div className="symbol-editor-preview">
          <img
            ...
            height={220}
          />
          <p>{feature.name || '未命名要素'}</p>
          <p className="muted">SIDC：{feature.sidc}</p>
        </div>
      ) : (
        <div className="symbol-editor-about">
          <h3>符号标准与 MSS</h3>
          <p>本地目录基线：MIL-STD-2525C；内部编辑编码兼容 2525D 数字 SIDC。</p>
          <p>MSS 版本：2019.10.01（原生 MilX V3.1）。</p>
          <p>
            工作模式：{mode === 'extended' ? 'Extended（最多两个扩展图标修饰符）' : 'International'}
            。
          </p>
          {nativeExternal && (
            <p className="native-format-warning">当前要素保留原生 15 位 SIDC 和 MSS 属性。</p>
          )}
        </div>
      )}
    </div>
