...
- [x] 3D 离线首帧与底图：默认采用椭球高程基线、持续首帧渲染和 Cesium Natural Earth 本地瓦片，修复无网络环境进入三维后纯蓝/纯色画布的问题；自托管真实 DEM 仍通过配置启用。
...
- [ ] 真实 DEM 离线部署：提供可再分发的 quantized-mesh/heightmap 数据包、瓦片完整性校验和高程回归截图；当前默认椭球基线不宣称包含真实地形高程。
