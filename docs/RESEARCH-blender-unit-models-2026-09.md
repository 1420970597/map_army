...
本轮把“挂载”暂按飞机外挂部件、传感器、油箱等装配理解。如果指“在舰船上增加舰载飞机”或“给编制分配下属单位”，则应使用载运/编制关系，不能直接复用外挂插槽语义。当前只提供可视化装配，不推导真实任务载荷或作战能力。
...
| [`package.json`](../package.json) | React 19、Zustand、Cesium | 评估开始时尚无 Three.js；续作已添加该依赖及 glTF Validator |
...
| 阶段 | 输出与通过条件 | 条件性投入 |
| --- | --- | --- |
| 工具验证 | Blender 可运行，生成或读取模型，导出 GLB 并检查挂点 metadata | 实测结果见下节，续作还增加了示意资产的应用集成 |
