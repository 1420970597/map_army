...
| R26 | 四、底图与视图 | 3D 视图 | 高度升降、航向/俯仰控制、图层可见性与透明度面板 | 无 | **建议关闭，不做** —— 详见 §3 专项评估 | P2（建议不做） | 高 | 否 |
...
| 当前栈能力 | Leaflet 1.9 为纯 2D 栅格/矢量引擎，**无相机俯仰与真三维坐标管线**，无法承载 |
| 改造代价 | 引入 Cesium/MapLibre-GL 等于替换地图内核，`src/features/map` 与军网叠加全部重写，且与现有 2525 符号渲染器不兼容 |
| 降级方案可行性 | 「2.5D 伪三维 / 倾斜视角」在 Leaflet 下只能做 CSS `transform: rotateX()`，会破坏 Leaflet 的坐标反算与命中检测，**不可用于标图** |
| **产品建议** | **关闭 R26，不实现**。相关用户价值由 R28（地形晕渲）与文档中「本项目为 2D 标图工具」的明确定位承担 |
| 后续 | 若主理人坚持，应作为**独立技术预研议题**（换内核 + 重做渲染层），不进入本路线图七个批次 |

> 结论：R26 保留在需求池中仅用于**记录差距与决策痕迹**，不排期、不估算工时。
