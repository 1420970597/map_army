...
| **明确排除** | README「不在复刻范围」OOS-01–OOS-09（Pro 时间轴、BFT、GPS 追踪器、用户管理登录、自定义 WMTS/WMS、口令保护分享、封闭网络部署、MSS 托管服务、用户论坛/作品征集） | OOS-01–OOS-07 与 OOS-09 不进免费版需求池；OOS-08 改为本地替代引擎的兼容限制，详细证据见 [范围审计](RESEARCH-out-of-scope-2026-09.md) |
| 技术不可行 | 3D 视图（R26） | 见 §3 专项评估，建议**关闭**，价值由 R28 承担 |
| 项目自有服务端降级 | R46（编辑覆盖）、R50（在线图层分享）、R61（作品提交）、R49（兵棋推演） | 可使用本项目 Share API 或静态页面做明确标注的替代方案；不得宣称具备 OOS-04/OOS-06/OOS-07 的用户身份、安全和闭网能力 |
...
| 红线 | **不得把项目自有 Share API 伪装成原站 Pro 身份/安全服务**；不实现口令保护、用户账户或闭网商业部署；R50 可使用项目 API 保存公开在线源 URL，并明确源站/CORS 限制 |
...
| 全局红线 | 不实现 OOS-01–OOS-07 与 OOS-09；OOS-08 只使用本地替代引擎；项目自有 Share API 仅支持免费版公开分享语义，不引入原站 Pro 身份/口令/闭网服务；分支名不含斜杠 |
