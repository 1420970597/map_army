...
| 轮次 | 范围                                                                                                   | 完成判据                                                            | 状态                                  |
| ---- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------- | ------------------------------------- |
| 1    | 图层管理闭环：定位全部/单层、过滤、单层导出、指定目标层导入、锁定语义、无变化历史                      | 纯逻辑和 store 测试通过；浏览器可操作；Compose/CI 全绿              | 已关闭                                |
| 2    | 原生 MilX：V3.1 Layer 结构、15 位 SIDC、MSS 属性、坐标系元数据、XML/ZIP 往返                           | 官方样例逐字段往返；不支持字段有提示；导出 ZIP 可被独立解析         | 已关闭                                |
| 3    | 分享闭环：空 Share、图层加入/移除、Edit & Copy 独立副本、版本固定、冲突提示、在线层元数据、iframe 预览 | 服务端并发/版本测试；三个权限模式浏览器冒烟；离线/在线边界清楚      | 已关闭                                |
| 4    | 符号与编辑器：2525C 基线标识、Edit/Preview/About、修饰符校验、Icon Extension、Extended 瑞士符号        | 符号目录/编辑器回归；15 位 SIDC 不退化；预览与导出一致              | 已关闭                                |
| 5    | 地图与平台：六边形网格高级配置、DEM/AMSL 能力边界、PWA 离线恢复、示例画廊与浏览器矩阵                  | Chromium/Firefox 自动化；移动触摸与离线恢复；站点示例可打开并可复现 | 六边形/PWA 子项已关闭，余项进入第六轮 |
...
本轮完成六边形网格边长、颜色、不透明度、线宽、标签和交点吸附配置，配置通过 `map-army.prefs.v1`
持久化；绘制与吸附使用同一列行坐标，避免视觉网格和吸附顶点错位。PWA 增加网络离线/恢复提示、文件关联
API 缺失时的导入按钮降级提示，并限制瓦片缓存为图像、单项 2 MiB、总量 500 条。

本轮验收：`npm run ci` 通过（62 个测试文件、975 个 Vitest 测试、3 个服务端测试），
`npx playwright test e2e/pwa-smoke.spec.ts` 的 Chromium 用例通过；Docker Compose 两个服务 healthy，
首页返回 200，`/api/health` 返回 `{"status":"ok"}`。原站复核证据和下一轮差异见
[TODO-original-site-audit-2026-09.md](TODO-original-site-audit-2026-09.md)。
