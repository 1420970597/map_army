...
from .documents import ASSET_PATTERN, references
...
def inspect_glb(data):
    ...
    try:
        ...
        raise ValueError("GLB 元数据无效") from exc
...
def model_metadata(db, owner_id, payload, asset):
    ...
    require(isinstance(sockets, list), "挂点配置无效")
    require(
...
def import_model(body: dict, request: Request, db: Session = Depends(session, scope="function")):
    ...
    require(text(body.get("name", asset.name)), "模型名称无效")
    job = Job(
